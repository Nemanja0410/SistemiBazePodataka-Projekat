    ...
    public partial class DTOManager
    {
        ...
        public static void odobriNalog(int nalogId, string dodeljenaUloga)
        {
            ...
            try
            {
                ...
                {
                    s.Close();
                    throw new InvalidOperationException("Status admin naloga se ne može menjati iz ovog ekrana.");
                }
                if (n.StatusNaloga != "NA_CEKANJU")
                {
                    s.Close();
                    throw new InvalidOperationException("Samo nalozi na čekanju mogu biti odobreni.");
                }
                if (dodeljenaUloga != "ADMIN" && n.Osoblje != null && n.Osoblje.TipOsoblja != dodeljenaUloga)
                {
                    s.Close();
                    throw new InvalidOperationException("Dodeljena uloga se mora poklapati sa tipom zaposlenog.");
                }
                n.Uloga          = dodeljenaUloga;
                n.StatusNaloga   = "ODOBREN";
                n.DatumOdobrenja = DateTime.Now;
                n.OdobrioNalog   = SesijaKorisnik.TrenutniNalog != null
                    ? s.Load<Nalog>(SesijaKorisnik.TrenutniNalog.NalogId)
                    : null;
                s.Update(n);
                s.Flush();
                s.Close();
        ...
        public static void odbijNalog(int nalogId)
        {
            ...
            try
            {
                ...
                {
                    s.Close();
                    throw new InvalidOperationException("Admin nalozi ne mogu biti odbijeni.");
                }
                if (n.StatusNaloga != "NA_CEKANJU")
                {
                    s.Close();
                    throw new InvalidOperationException("Samo nalozi na čekanju mogu biti odbijeni.");
                }
        ...
        public static void zakljucajNalog(int nalogId)
        {
            ...
            try
            {
                ...
                {
                    s.Close();
                    throw new InvalidOperationException("Admin nalozi ne mogu biti zaključani.");
                }
                if (n.StatusNaloga != "ODOBREN")
                {
                    s.Close();
                    throw new InvalidOperationException("Samo odobreni nalozi mogu biti zaključani.");
                }
                n.StatusNaloga = "ZAKLJUCAN";
                s.Update(n);
                s.Flush();
        ...
        public static void otkljucajNalog(int nalogId)
        {
            ...
            try
            {
                ...
                {
                    s.Close();
                    throw new InvalidOperationException("Admin nalozi ne mogu biti otključani iz ovog ekrana.");
                }
                if (n.StatusNaloga != "ZAKLJUCAN")
                {
                    s.Close();
                    throw new InvalidOperationException("Samo zaključani nalozi mogu biti otključani.");
                }
                n.StatusNaloga      = "ODOBREN";
                n.NeuspesnihPrijava = 0;
                s.Update(n);
        ...
        public static void obrisiNalog(int nalogId)
        {
            ...
            try
            {
                ...
                {
                    s.Close();
                    throw new InvalidOperationException("Admin nalozi ne mogu biti obrisani iz ovog ekrana.");
                }
                s.Delete(n);
                s.Flush();
                s.Close();
